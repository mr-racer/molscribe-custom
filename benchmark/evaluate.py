...
def score(gold_rows, preds, kind):
    ...
    for r in gold_rows:
        ...
        for src in ("all", r["source"].split("-")[0]):
            d = per_source[src]
            d["n"] += 1
            for k in ("stereo", "nostereo", "valid", "gold_ok"):
                d[k] += int(res[k])
...
def main():
    ...
    for run_dir in args.pred:
        ...
        for path in sorted(glob.glob(os.path.join(run_dir, "*_bs*.csv"))):
            ...
            print(f"{run:28s} {name:16s} n={m['n']:4d}  EM stereo={m['em_stereo']:.3f}  "
                  f"EM no-stereo={m['em_nostereo']:.3f}  valid={m['valid']:.3f}  "
